...
namespace trackerTFP {
  ...
  private:
    ...
    // ED output token for lost objects
  ...
  ProducerGP::ProducerGP(const ParameterSet& iConfig) : iConfig_(iConfig) {
    const string& label = iConfig.getParameter<string>("LabelPP");
  ...
  void ProducerGP::produce(Event& iEvent, const EventSetup& iSetup) {
    ...
    StreamsStub tuncated(numRegions * numChannelOut);
    ...
    auto put = [](const deque<StubGP*>& objects, StreamStub& stream) {
    ...
    for (int region = 0; region < numRegions; region++) {
      ...
      vector<deque<StubGP*>> streamsTrunc(numChannelOut);
      // fill output data
      gp.produce(streamsIn, streamsOut, streamsTrunc);
      ...
      for (int channelOut = 0; channelOut < numChannelOut; channelOut++) {
        ...
        put(streamsTrunc[channelOut], tuncated[index]);
      }
    }
    // store products
    iEvent.emplace(edPutTokenAccepted_, move(accepted));
    iEvent.emplace(edPutTokenTruncated_, move(tuncated));
  }
