...
void efficiencyComparisonBarChart () {
   ...
   if (!combinedOn) {
      ...
      switch(inJetSetting) {
         case 0:
            for (double i : {96.1977, 96.1178, 96.1791, 96.1605, 95.7747, 95.5087, 95.6393, 96.1948, 96.1977}) {
               hybrid.push_back(i);
            }
            for (double i : {95.877, 95.8282, 95.8593, 95.8388, 95.4816, 95.4922, 95.541, 95.877, 95.877}) {
               newkf.push_back(i);
            }
            hybridFull = 94.8894;
            hybridNo = 96.1977;
         ...
         case 1:
            ...
            for (double i : {95.8983, 95.8442, 95.8727, 95.8461, 95.3907, 95.3976, 95.4536, 95.8983, 95.8983}) {
               newkf.push_back(i);
            }
            hybridFull = 94.572;
            hybridNo = 96.2799;
         ...
         case 2:
            ...
            for (double i : {95.4892, 95.4028, 95.4306, 95.3473, 94.3476, 94.2334, 94.2396, 95.4892, 95.4892}) {
               newkf.push_back(i);
            }
            hybridFull = 91.9827;
            hybridNo = 96.1617;
         ...
         case 3:
            ...
            for (double i : {94.9319, 94.8245, 94.8603, 94.5022, 92.0487, 90.8309, 90.7951, 94.9319, 94.9319}) {
               newkf.push_back(i);
            }
            hybridFull = 84.1304;
            hybridNo = 95.5925;
   ...
   vector <double>hybridFullCombinedTextY = {0.515, 0.515, 0.44, 0.22};
   vector <double>hybridNoCombinedTextY = {0.637, 0.595, 0.615, 0.57};
   vector <double>newkfFullCombinedTextY = {0.60, 0.595, 0.53, 0.37};
   vector <double>newkfNoCombinedTextY = {0.695, 0.705, 0.705, 0.69};

   vector <double>hybridFullTextY = {0.61, 0.60, 0.47, 0.18};
   vector <double>hybridNoTextY = {0.74, 0.74, 0.74, 0.72};
   vector <double>newkfFullTextY = {0.61, 0.60, 0.47, 0.3};
   vector <double>newkfNoTextY = {0.74, 0.74, 0.74, 0.72};

   double hybridTextX = 0.13;
   double newkfTextX = 0.4;

   double hybridCombinedTextX = 0.31;
   double newkfCombinedTextX = 0.54;

   char ctxt1[500];
   sprintf(ctxt1, "Hybrid Full Trunc"); // Add label saying 
   if (combinedOn) {
      mySmallText(hybridCombinedTextX, hybridFullCombinedTextY[inJetSetting], kOrange-3, ctxt1);
   }
   else {
      mySmallText(hybridTextX, hybridFullTextY[inJetSetting], kOrange-3, ctxt1);
   }
   
   // y vals: 
   // combined: 0-0.515 1-0.515 2-0.44 3-0.22

   char ctxt2[500];
   sprintf(ctxt2, "Hybrid No Trunc"); // Add label saying 
   if (combinedOn) {
      mySmallText(hybridCombinedTextX, hybridNoCombinedTextY[inJetSetting], kOrange-3, ctxt2);
   }
   else {
      mySmallText(hybridTextX, hybridNoTextY[inJetSetting], kOrange-3, ctxt2);
   }
   // y vals: 
   // combined: 0-0.637 1-0.595 2-0.615 3-0.57

   char ctxt3[500];
   sprintf(ctxt3, "NewKF Full Trunc"); // Add label saying 
   if (combinedOn) {
      mySmallText(newkfCombinedTextX, newkfFullCombinedTextY[inJetSetting], kGreen+1, ctxt3);
   }
   else {
      mySmallText(newkfTextX, newkfFullTextY[inJetSetting], kGreen+1, ctxt3);
   }
   // y vals: 
   // combined: 0-0.60 1-0.595 2-0.53 3-0.37

   char ctxt4[500];
   sprintf(ctxt4, "NewKF No Trunc"); // Add label saying 
   if (combinedOn) {
      mySmallText(newkfCombinedTextX, newkfNoCombinedTextY[inJetSetting], kGreen+1, ctxt4);
   }
   else {
      mySmallText(newkfTextX, newkfNoTextY[inJetSetting], kGreen+1, ctxt4);
   } 
   // y vals: 
   // combined: 0-0.695 1-0.705 2-0.705 3-0.69

   cb->Update();

   TString inJetString = to_string(inJetSetting);
   cb->SaveAs(saveDir + "effComparison_barChart_" + inJetString + ".pdf");
}
