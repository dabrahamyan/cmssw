...
void overlayHists_resVsEta_newkfTest (){
    ...
    vector <TString> labels = {"TTbar PU=200"};
    
    // doing this manually bc max does not work w resVsEta plots for some reason
    vector <float> maxVals = {0.035, 0.12, 0.006, 2.4};
    ...
    for (int iDataSet = 0; iDataSet < dataSets.size(); iDataSet++) {
        ...
        TFile *newkfFile= new TFile(dir + "output_hybridvsnewkf_test1_1_numEvent10000.root");
        ...
        for (int iParam = 0; iParam < params.size(); iParam++) {
            ...
            leg->Draw();
            c.SaveAs(saveDir + "test1" + "_" + prop + params[iParam] + ".pdf");

            /////////////////////// DEBUG CODE ////////////////////////////////////
            // cout << "Entries in Old KF " + labels[iDataSet] + " 68%: " << newkfHist68->GetEntries() << endl;
            // cout << "Entries in New KF " + labels[iDataSet] + " 68%: " << newkfHist68->GetEntries() << endl;
            // cout << "Entries in Old KF " + labels[iDataSet] + " 90%: " << newkfHist90->GetEntries() << endl;
