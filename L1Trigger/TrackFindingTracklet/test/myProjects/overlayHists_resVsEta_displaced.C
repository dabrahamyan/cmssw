...
void overlayHists_resVsEta_displaced (){
    ...
    TString label = "Displaced Muon PU=0";
    TString label2 = "Hybrid Displaced Tracking";
    
    // doing this manually bc max does not work w resVsEta plots for some reason
    vector <float> maxVals = {0.015, 0.7, 0.02, 3};
    ...
    // For data sets like SingleMuon, SingleElectron
        // Load newkf and newkf root files
        TFile *newkfFile= new TFile(dir + fileName + ".root");

        // For parameters like eta, phi, ...
        for (int iParam = 0; iParam < params.size(); iParam++) {
            // copy newkf and newkf hists for param
            ...
            //vector<TH1*> histList {newkfHist68, newkfHist68, newkfHist90, newkfHist90};
            //max = GetMaxHists(histList);
            newkfHist68->GetYaxis()->SetRangeUser(0, maxVals[maxCounter]);
            maxCounter++;

            // make legend
            TLegend* leg = new TLegend(0.2, 0.65, 0.45, 0.85);
            leg->AddEntry(newkfHist68, "Res=68%", "p"); 
            leg->AddEntry(newkfHist90, "Res=90%", "p");  
            
            // Draw and save histos
            newkfHist68->Draw("p");
            newkfHist90->Draw("p same");
            sprintf(ctxt, label); // Add label saying 
            mySmallText(0.47, 0.8, 1, ctxt); // which data set it is
            sprintf(ctxt2, label2); // Add label saying 
            mySmallText(0.47, 0.85, 1, ctxt2); // which data set it is
            leg->Draw();
            c.SaveAs(saveDir + fileName + "_" + params[iParam] + ".pdf");
