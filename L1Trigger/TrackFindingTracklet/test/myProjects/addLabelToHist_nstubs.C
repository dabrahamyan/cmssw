...
void addLabelToHist_nstubs (){
    ...
    mySmallText(0.6, 0.76, 1, ctxt);    
    c.SaveAs(saveDir + prop + "_SingleMuonPU0.pdf");

    electronHist->Draw(); 
    sprintf(ctxt, "Single Electron PU=0");
    mySmallText(0.6, 0.76, 1, ctxt);    
    c.SaveAs(saveDir + prop + "_SingleElectronPU0.pdf");

    TTbarPU200Hist->Draw(); 
    sprintf(ctxt, "TTbar PU=200");
    mySmallText(0.25, 0.76, 1, ctxt);    
    c.SaveAs(saveDir + prop + "_TTbarPU200.pdf");

    TTbarPU0Hist->Draw(); 
    sprintf(ctxt, "TTbar PU=0");
    mySmallText(0.25, 0.76, 1, ctxt);    
    c.SaveAs(saveDir + prop + "_TTbarPU0.pdf");
