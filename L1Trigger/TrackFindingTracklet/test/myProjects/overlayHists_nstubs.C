...
void overlayHists_nstubs (){
    ...
    // Plot
    TLegend* leg = new TLegend(0.23, 0.4, 0.48, 0.6);
    ...
    muonHist->Draw("l");
    electronHist->Draw("l same");
    TTbarPU200Hist->Draw("l same");
    TTbarPU0Hist->Draw("l same");
    leg->Draw();
    max = GetMaxHists(allHists);
    muonHist->GetYaxis()->SetRangeUser(0, max*1.1);
    muonHist->GetXaxis()->SetRangeUser(0, 10);
    muonHist->GetYaxis()->SetTitle("Probability Density of Tracks / 1");
    ...
    delete muonFile, electronFile, TTbarPU200File, TTbarPU0File;
}
